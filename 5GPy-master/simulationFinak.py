...
#TODO change the manual setting of the size of packets into a variable created during the setting of the simulation (we are using the value of 128 so far)
...
class BaseSensor(object):
	...
	def run(self):
		...
		while True:
			...
			packet = BasePacket(packetId, self.aId, "Normal", 128)#96  is a value in kb for the size of the packet 
	...
	def sendData(self, sendTime):
		...
		while True:
			...
			if self.packets:#verify if there is data/packets to send
				...
				iotPkt = iotPacket(packetId, self.aId, "IoT", len(self.packets)*128, self.packets.copy())#96  is a value in kb for the size of the packet 
...
class TemperatureSensor(BaseSensor, object):
	...
	def run(self):
		...
		while True:
			yield self.env.timeout(self.generationInterval)#wait a given time to generate another packet/data
			packet = BasePacket(packetId, self.aId, "Normal", 120)#96  is a value in kb for the size of the packet 
	...
	def sendData(self, sendTime):
		...
		while True:
			...
			if self.packets:#verify if there is data/packets to send
				#create a "iot" packet containing all generated data/packets so far
				iotPkt = iotPacket(packetId, self.aId, "IoT", len(self.packets)*120, self.packets.copy())#96  is a value in kb for the size of the packet 
				self.iotGateway.aggregatedData.put(iotPkt)#"send" it to the iot gateway connected to this sensor
				packetId += 1#increment the counter of packets
				self.packets = []#clean the stored packets in this sensor
...
class HumididtySensor(BaseSensor, object):
	...
	#put the sensor to run
	def run(self):
		self.setHumidity = random.randrange(0,60)
		packetId = 0#counter to identify each generated packet in this sensor
		while True:
			yield self.env.timeout(self.generationInterval)#wait a given time to generate another packet/data
			packet = BasePacket(packetId, self.aId, "Normal", 120)#192  is a value in kb for the size of the packet 
			print("Sensor {} of Type {} generated Packet: ID {} Type {} Size {} at moment {} and the value {}\n".format(self.aId, self.aType, packet.aId, packet.aType, 
...
class ProcessingNode(BaseNode, object,):
		def __init__(self, env, aId,aType,iotGateway):
			self.env = env
			self.aType = aType
			self.aId = aId
...
sensor_Temperature = TemperatureSensor(env, 0, "Temperature",iotGateway1,generationInterval,sendInterval)#create temperature sensor
sensor_Humidity1 = HumididtySensor(env, 0, "Humidity", iotGateway1, generationInterval, sendInterval)
sensor_Ph1 = PhSensor(env, 0, "Ph", iotGateway1, generationInterval, sendInterval)

ProcessingNode1 = ProcessingNode(env, 0, "Cloud",iotGateway1)
