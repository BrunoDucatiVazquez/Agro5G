...
class BaseIoTGateway(object):
	def __init__(self, env, aId):
		...
		self.transmissionRate = self.env.timeout(self.totalSinglePackets/self.capacity) # simulate the transmission rate of an IoT antenna
...
class antenna5gGateway(BaseIoTGateway,object):
	def __init__(self, env, aId):
			...
			self.transmissionRate = self.env.timeout(self.totalSinglePackets/self.capacity) # simulate the transmission rate of an 5G antenna
	def run(self):
...
class SoilTemperatureSensor(BaseSensor, object):
	...
	def run(self):
		packetId = 0#counter to identify each generated packet in this sensor
		while True:
			self.setTemperature = random.randrange(0,40) # simulate the value of the sensor
			yield self.env.timeout(self.generationInterval)#wait a given time to generate another packet/data
...
class SinkNode(BaseNode, object):
