...
class BaseIoTGateway(object):
	def __init__(self, env, aId):
		...
		self.transmissionRate = self.env.timeout(self.totalSinglePackets/104857600)
	...
	def run(self):
		...
		while True:
			...
			print("Dado chegando no IoT gateway at moment {}\n".format(self.env.now))
...
class antenna5gGateway(BaseIoTGateway,object):
	def __init__(self, env, aId):
			...
			self.transmissionRate = self.env.timeout(self.totalSinglePackets/50000000000)
	def run(self):
			print("...5G  gateway {} is running and receiving data from sensors...")
			while True:
				#waits to receive data from a sensor
				z = yield self.aggregatedData.get()
				print("Dado chegando no 5G gateway at moment {}\n".format(self.env.now))
...
# soil_sensor_Temperature1 = SoilTemperatureSensor(env, 0, "Temperature",iotGateway1,generationInterval,sendInterval,)#create temperature sensor
# sensor_Humidity1 = HumididtySensor(env, 0, "Humidity", iotGateway1, generationInterval, sendInterval)
# sensor_Water_Level1 = WaterLevelSensor(env, 0, "WaterLevel", iotGateway1, generationInterval, sendInterval)

ProcessingNode = ProcessingNode(env, 0, "Cloud",antenna5gGateway,FuzzyLogic)

# ProcessingNode = ProcessingNode(env, 0, "Cloud",iotGateway1,FuzzyLogic)
